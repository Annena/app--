define('page/storedValue', [
    ...
    // 储值页面
    StoredValue.prototype = new Page('storedValue');

    StoredValue.prototype.util = function() {
        ...
        var Stored = {
            ...
            init: function() {
                ...
                if (!$.cookie("user_mobile")) {
                    ...
                    $('#stored_details').addClass('hide');
                } else {
                    // 领取会员卡
                    this.membershipCard();
                    // 显示账户信息，隐藏未登录
                    $('#stored_details').removeClass('hide');
                ...
                if (yesNoCard == '' || yesNoCard == undefined) {
                    ...
                    Data.setAjax('companyCard', {
                        ...
                        if (respnoseText.code == 20) {
                            ...
                            if (scanCodeType != undefined && scanCodeType == 0 && scanType == 3) {
                                // 储值
                                Stored.recharge(scanCodeRecordId);
                            } else {
                                // 显示账户储值信息
                ...
                Data.setAjax('accountInfo', {
                    ...
                    if (respnoseText.code == 20) {
                        ...
                        if ($.cookie("user_mobile")) {
                            ...
                            //储值卡范围
                             storedRang =  data.account_info.currency_stored;
                            ...
                            if (paystored == 1) {
                                Message.show('#msg', '储值成功', 2000);
                            } else if (paystored == 2) {
                                Message.show('#msg', '储值失败，请从新尝试', 2000);
                            }
                            if (cardId == '' && paystored != undefined) {
                                window.location = 'http://www.lekabao.net/vxtest/html/index.html';
                            }
                        }
                        // 显示储值卡列表
                        self.storedListDate(data.stored_list, data);
                        // 显示储值范围
                        self.storedRangData(storedRang);
                    } else {
                        Message.show('#msg', respnoseText.message, 2000);
                    }
                }, 2);
            ...
            //显示储值范围
            storedRangData: function(data){
                var self = this;
                if( data.length == 0){
                    $('.storedvalue-rang').css('display','none');
                    $('.storedvalue-valuenews-circle').addClass('hide');
                    // $('.pg-storedvalue .storedvalue-titlebig span').css('padding-top','0.5rem');
                    $('.pg-storedvalue .storedvalue-valuenews-pitch').css('height','12rem');
                }else{
                    $('.storedvalue-rang').css('display','block');
                    $('.pg-storedvalue .storedvalue-valuenews-pitch').css('height','220px');
                    // this.scroll.refresh();
                    // $('.pg-storedvalue .storedvalue-titlebig span').css('padding-top','0');
                    var rangContent = data.toString();    //.replace(//g,'、');
                    if(rangContent.length > 12){
                        rangContent = '<p>储值通用范围：'+rangContent.substr(0,9).replace(/,/g,'，')+'<span id="lookRang"> >> </span></p>';
                        $('.storedvalue-valuenews-circle').removeClass('hide');
                    }else{
                        rangContent = '<p>储值通用范围：' + rangContent.replace(/,/g,'，')+  '</p>';
                        $('.storedvalue-valuenews-circle').addClass('hide');
                    }
                    $('.storedvalue-rang').html(rangContent);
                }
                setTimeout(function () {
                    self.scroll.refresh();
                }, 1000);
            ...
            bindStoredClick: function() {
                ...
                // 输入框输入值改变进行搜索
                /*$('#changeInput').unbind('input').bind('input', function () {
                    self.searchDate($('#changeInput').val());
                });*/
...
//     alert($(this).html());
// });
                $('.storedvalue-titlebig-rang').on('click','span#lookRang', function() {
                    $(".storedvalue-titlebig-rang").addClass('hide');
                    $(".storevalueRang").removeClass('hide');
                    $('.circleW').css('background','#fb8a2c');
                    $('.circle').css('background','#fff');
                    //全部范围
                    var rangContentAll = storedRang.toString();
                    if(rangContentAll.length > 80){
                        rangContentAll = rangContentAll.substr(0,80)+'...';
                    }
                    $('.storevalueRang-content').html('<p>'+ rangContentAll + '</p>');
                });

                //储值通用范围返回rangCallback
                $('.rangCallback').unbind('click').bind('click', function() {
                // $('.storedvalue-titlebig-rang').on('click','.rangCallback', function() {
                    $(".storedvalue-titlebig-rang").removeClass('hide');
                    $(".storevalueRang").addClass('hide');
                    $('.circleW').css('background','#fff');
                    $('.circle').css('background','#fb8a2c');
                });

                //点击储值记录页面
                $('#storevalueList').unbind('click').bind('click', function() {
                    // if (merData.is_del == 1) {
                    //     return;
                    // } else {
                        Page.open('storedValueRecordList&card_id=' + cardId + '&page=storedValue');
                ...
                // 未登录，点击页面登陆按钮
                $('#stored_login').unbind('click').bind('click', function() {
                    Cache.set('loginReturn', location.href.split('?')[1]);
                    Page.open('login');
                });
                // 未登录，点击页面注册按钮
                $('#stored_register').unbind('click').bind('click', function() {
                    Cache.set('loginReturn', location.href.split('?')[1]);
                    Page.open('register&page=storedValue');
                });
            },

            // 输入储值码储值
            /*searchDate: function (date) {
                Stored.recharge($('#input-purchase').val());
            },*/
            ...
            weixinVisit: function() {
                ...
                if (isWeixin || isAli) {
                    ...
                    $('.storedvalue-nav').addClass('top45')
    ...
    StoredValue.prototype.bindPageEvents = function() {
        var self = this;
        // 微信打开
